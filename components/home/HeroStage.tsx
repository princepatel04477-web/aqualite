...
export function HeroStage({
  ...
  useGSAP(
    () => {
      ...
      if (allowCursorFX && parallax) {
        const xTo = gsap.quickTo(parallax, "x", {
          duration: duration.base,
          ease: gsapEase.tide,
        });
        const yTo = gsap.quickTo(parallax, "y", {
          duration: duration.base,
          ease: gsapEase.tide,
        });
  ...
  return (
    <section
      ...
      className="relative -mt-[calc(var(--header-h)+2rem)] overflow-hidden bg-abyss"
    >
      <TideField className="absolute inset-0 h-full w-full" />
      <div className="hero-glow pointer-events-none absolute inset-0" />
      <div className="hero-vignette pointer-events-none absolute inset-0" />

      {/* ── Desktop hero — approved choreography, untouched above 1024px ── */}
      <div
        data-hero="desktop"
        className="relative z-[2] hidden min-h-[100svh] flex-col justify-end lg:flex"
      >
        <div data-scroll className="pointer-events-none absolute inset-0">
          <div
            data-parallax
            className="absolute inset-x-0 top-[4%] flex justify-center lg:top-[0%]"
          >
        ...
        <div className="page-wrap pb-8 lg:pb-10">
          ...
          <div className="mt-8 flex flex-col gap-6 border-t border-hairline pt-5 sm:flex-row sm:items-end sm:justify-between">
            <Link href={productHref} data-meta className="group">
              ...
              <p className="tabular mt-1 font-body text-h3 text-foam transition-colors duration-quick ease-tide group-hover:text-sand">
                {price}
              </p>
            </Link>
            ...
            <div
              data-meta
              className="hidden items-center gap-3 font-mono text-eyebrow uppercase text-mist sm:flex"
            >
              <span>Scroll</span>
              <span className="relative h-12 w-px overflow-hidden bg-hairline">
                <span
                  data-scroll-line
                  className="absolute inset-x-0 top-0 h-1/2 bg-aqua"
                />
              </span>
            </div>
          </div>
      ...
      {/* ── Mobile hero — purpose-built for phones (M06): eyebrow → headline
          (CSS fade/rise, no SplitText) → shoe 78vw at half-amplitude float
          → one-sentence lead → side-by-side 48px CTAs. The product strip is
          intentionally dropped: with it the first viewport overflows a
          375×667 iPhone SE, without it everything fits with room to spare. ── */}
      <div className="page-wrap relative z-[2] flex min-h-[100svh] flex-col justify-end pb-6 pt-[calc(var(--header-h)+1.25rem)] lg:hidden">
        <div className="hero-mline">
          <Eyebrow index="01" total="06">
            {eyebrow}
          </Eyebrow>
        </div>
        <h1 className="heading-display mt-3 font-display text-display font-normal text-foam">
          <span
            data-mline
            className="hero-mline block"
            style={{ animationDelay: "calc(var(--dur-quick) * 0.6)" }}
          >
            {lead}
          </span>
          <span
            data-mline
            className="hero-mline block"
            style={{ animationDelay: "calc(var(--dur-quick) * 1.3)" }}
          >
            <em>{emphasis}</em> {rest}
          </span>
        </h1>
        ...
        >
          ...
          <img
            ...
            height={1000}
            fetchPriority="high"
            className="hero-shoe w-[78vw] max-w-[21rem]"
          />
        </Buoyancy>
        <p className="mt-1 text-lead text-mist">{firstSentence(copy)}</p>
