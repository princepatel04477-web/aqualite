...
export function TideField({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Only the approved high tier animates the field; mobile/touch,
    // low tier and reduced motion get a static gradient poster frame
    // painted once (M06 — no rAF loop on the first phone viewport).
    const animated = !reduced && tier === "high";
    const dpr = Math.min(
