...
export function generateStaticParams() {
  return GENDERS.filter((gender) => gender !== "unisex").map((gender) => ({
    gender,
  }));
}
...
export default async function GenderPage({
  ...
  if (!GENDERS.includes(params.gender as Gender) || params.gender === "unisex")
    notFound();
  const parsed = parseListing(searchParams);
  const [listing, scope] = await Promise.all([
    listProducts({ ...parsed, gender: params.gender as Gender }),
    listScope(parsed),
  ]);
  const label = params.gender[0]?.toUpperCase() + params.gender.slice(1);
  return (
    <ListingView
      ...
      listing={listing}
      scope={scope}
      crumbs={[{ href: "/shop", label: "Shop" }, { label }]}
      title={
        <>
          {label}&apos;s <em>edit</em>
        </>
